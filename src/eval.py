...
try:
    ...
    USE_WANDB = True
    PROJECT_NAME = 'something'  # Please specify the project name.
except Exception:
...
def parse_args():
    ...
    parser.add_argument('--gpu_idx', type=str, default='0', help="which gpu is used")
    ...
    parser.add_argument("--eval_max_steps", default=150, type=int, help="Max steps allowed in eval.")
...
if __name__ == "__main__":
    ...
    if args.task in ["TurnFaucet-v0", "PegInsertionSide-v0"]:
        args.eval_max_steps = 250
    if args.task in ["StackCube-v0"]:
        args.eval_max_steps = 200
    USE_WANDB =False
    ...
    n_env = 20  # Number of parallel environments.
    ...
    env_test = gym.make(json_data['env_info']["env_id"], **json_data['env_info']["env_kwargs"])
    env_test.reset()
    metric_dict = defaultdict(lambda: [[] for _ in range(len(eval_ids))])
    for start_idx in tqdm(range(0, len(eval_ids), n_env)):
        print(args.from_ckpt, "start_idx: ",start_idx)
        reset_args_list = []
    ...
    print(output_str)
    append_to_file(filename, f"model: {args.model_name}; iter: {args.from_ckpt}; {output_str}")
