"""
Code for the model architecture, based on CoTPC and the GPT implementation.
...
class KFE_m(nn.Module):
    def __init__(self, input_dim=768, output_dim=3, hidden_dims=[768,768,512,512], act_fn='relu', traj_len=60, concept_len=5, embed_size=5):
        super().__init__()
        self.position_embedding = torch.nn.Embedding(traj_len, embed_size)
        self.concept_embedding = torch.nn.Embedding(concept_len, embed_size)
        
        self.fcn = FCN(input_dim, 512, hidden_dims)
        self.drop = nn.Dropout(0.2)
        self.mlp = MLP_KFE(512,output_dim,[512,256,128])
        
    def forward(self, x, time_range, concept_idx):
        time_range = time_range.int()
        B, L, D = x.shape
        pe = self.position_embedding(time_range)
        te = self.concept_embedding(concept_idx) 
        x = torch.cat([x, pe.unsqueeze(0).expand(B, -1, -1), te.unsqueeze(1).expand(-1, L, -1)], dim=-1)
        x = x.transpose(2, 1)
        x = self.fcn(x)
        x = torch.max(x, 2, keepdim=True)[0].squeeze() # [B, 512]
        x = self.mlp(x)
        return x
...
class FCN(nn.Module):
    def __init__(self, input_dim, output_dim, hidden_dims=[], act_fn='relu'):
        ...
        for i, j in zip(dims[:-1], dims[1:]):
            layers.append(torch.nn.Conv1d(i, j, 1))
...
class MLP_VQ(nn.Module):
    def __init__(self, input_dim=768, output_dim=768, hidden_dims=[768], n_e=3, beta=1, act_fn='relu'):
        super().__init__()
        assert act_fn in ['relu', 'tanh', None, '']
        dims = [input_dim] + hidden_dims + [output_dim]
        layers = []
        ...
        self.embedding = nn.Embedding(self.n_e, self.output_dim)
                
    def forward(self, x):
        d = torch.sum(x ** 2, dim=1, keepdim=True) + \
            torch.sum(self.embedding.weight**2, dim=1) - 2 * \
            torch.einsum('bd,dn->bn', x, rearrange(self.embedding.weight, 'n d -> d n'))
        min_encoding_indices = torch.argmin(d, dim=1)
        z_q = self.embedding(min_encoding_indices)
        loss = self.beta * torch.mean((z_q.detach()-x)**2) + \
                   torch.mean((z_q - x.detach()) ** 2)
        z_q = x + (z_q - x).detach()
        return x, z_q, loss
...
class GPTWithCoT(nn.Module):
    ...
    def intermediate_blocks(self, states, timesteps, actions=None, key_state_mask=None):
    ...
    def forward(self, states, timesteps, actions=None, key_state_mask=None):
        ...
        global_pos_emb = torch.repeat_interleave(self.global_pos_emb, B, dim=0)
        timesteps_rp = torch.repeat_interleave(timesteps[:, None], self.config.n_embd, dim=-1)
        global_pos_emb = torch.gather(global_pos_emb, 1, timesteps_rp.long()) 
        local_pos_emb = torch.repeat_interleave(self.local_pos_emb, 2, dim=1) \
            if '+a' in self.model_type else self.local_pos_emb
