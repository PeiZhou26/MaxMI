...
def lookup_value_grid(matrix, vector, mode):
    ...
    dim = matrix.shape[1]
    num = vector.shape[1]
    matrix = matrix.view(batchsize, 1, dim, dim) # [100,1,1000,1000]
    vector = vector.view(batchsize, num, 1, 2) # [100,1005,1,2]
    value = (F.grid_sample(matrix, vector, mode, padding_mode='border', align_corners=True)).squeeze() # [100,1,1005,1]>[100,1005]
    return value   
...
def mutual_information(batch, matrix):
    ...
    et = torch.exp(torch.clamp(lookup_value_grid(matrix, marginal, "bilinear"), min=-50, max=50))
...
class PerceiverIO(nn.Module):
    ...
    def forward(
        ...
        latents = self.encoder(inputs, input_mask) # [B,sample nums,2] [B,256,256]
        query = self.query_gen(inputs) # [B,1000,1000]
